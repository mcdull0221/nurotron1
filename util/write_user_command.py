...
class WriteUserCommand:
    def read_data(self):
        '''加载yaml数据'''
        with open("F:/Python project/nurotron1/config/userConfig.yaml") as fr:
            data = yaml.load(fr)
    ...
    def write_data(self, i,device, bp, port):
        ...
        with open("F:/Python project/nurotron1/config/userConfig.yaml", 'a') as fr:
            yaml.dump(data, fr)
    ...
    def clear_data(self):
        with open("F:/Python project/nurotron1/config/userConfig.yaml", 'w') as fr:
            fr.truncate()
